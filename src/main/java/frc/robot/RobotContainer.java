...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    // PathConverter(m_drivetrain, "output/Course1.wpilib.json").getCommand());
    // m_chooser.addOption("Ramsete Trajectory - Course2", new
    // PathConverter(m_drivetrain, "output/Course2.wpilib.json").getCommand());
    // m_chooser.addOption("Ramsete Trajectory - 1 Ball top", new
    // PathConverter(m_drivetrain, "output/1ball_top.wpilib.json").getCommand());
    // m_chooser.addOption("Ramsete Trajectory - 1 Ball bottom", new
    // PathConverter(m_drivetrain, "output/1ball_bottom.wpilib.json").getCommand());

    // m_chooser.addOption("Ramsete Trajectory - COMMAND
    // GROUUUUUUUUUUUUUUPPPPPPPPPJ!!!!!!!!!!!!!!!!!!!", new SequentialCommandGroup(
    // new PathConverter(m_drivetrain,
    // "output/1ball_bottom.wpilib.json").getCommand(),
    // new InstantCommand(m_intakeMotor::openIntake, m_intakeMotor),
    // new PathConverter(m_drivetrain,
    // "output/bottomtopark.wpilib.json").getCommand(),
    ...
    // m_chooser.addOption("Auto Routine Distance", new
  ...
  public Command getArcadeDriveCommand() {
    return new ArcadeDrive(m_drivetrain, () -> -m_controller.getRawAxis(1) * 0.8,
        () -> m_controller.getRawAxis(0) * 0.7);
  }
}
